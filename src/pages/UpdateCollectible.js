...
class UpdateCollectible extends Component {
    constructor({match}) {
        super();
        this.state={
            data:[], id:parseInt(match.params.id), name:"", image_url:"", banner_img:"", col_symbol:"", current_supply:"", total_supply:"", token:"", loading:true, submitting:false,
            coins:[],
        }
    }
    ...
    componentDidMount() {
        ...
        Axios.get(ApiUrl.baseurl+"collectibles/"+this.state.id,formd)
            .then(res=>{
                if(res.data.error){
                    ...
                    this.setState({data:collectible,loading:false})
                }
            })
            .catch(err=>{
                console.log(err)
    ...
    onUpdate = (id, name, image, banner_img, total_supply, current_supply, price, currency_id) => {
        ...
        Axios.post(ApiUrl.baseurl + "update-collectible/" + id, formData)
            .then(response => {
                if (response.data.error) {
                    toast.error(response.data.error);
                } else {
                    toast.success("Collectible updated successfully");// Refresh the list
    ...
    handleSubmit = (e) => {
        e.preventDefault();
        const { id, name, image_url, banner_img, total_supply, current_supply, price, currency_id } = this.state.data;
        ...
        this.onUpdate(id, name, image_url, banner_img, total_supply, current_supply, price, currency_id);
    }
    ...
    render() {
        ...
        return (
            <Menu title="Update Collectible">
                <div className="container-fluid">
                    <div className="row">
                        <div className="col-md-12">
                            <div className="card rounded-0">
                                <div className="card-header">
                                    <h4 className="card-title">Update Collectible</h4>
                                </div>
...
export default UpdateCollectible;