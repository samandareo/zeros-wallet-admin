...
class EditStaking extends Component {
    constructor({match}) {
        super();
        this.state={
    ...
    async componentDidMount() {
        ...
        await Axios.get(ApiUrl.baseurl+"stake/"+this.state.id)
            .then(res=>{
                if(res.data.length==0){
                    ...
                    this.setState({
                        coin_raw_id:res.data[0]['coin_raw_id'],profit_coin:res.data[0]['profit_coin'],rate:res.data[0]['rate'],
                        days:res.data[0]['days'], profit:res.data[0]['profit'],status:res.data[0]['status'],
                        min_invest:res.data[0]["min_invest"],
                        loading:false
                    })
                }
    ...
    onSubmitForm=(event)=>{
        ...
        Axios.post(ApiUrl.baseurl+"updatestake", formData)
            .then(res=>{
                if(res.data.success){
    ...
    render() {
        ...
        return (
            <Menu title="Edit update Currency And Coin">
                <h3 className="bg-white title-head"> Edit Staking Coin </h3>
