...
class EditInfo extends Component {
    constructor({match}) {
        super();
        this.state={
            token:"",title:"",des:"",route:"",id:match.params.id,navigate:""
        }
        Axios.get(AppUrl.baseurl+"info/"+this.state.id)
            .then(res=>{
                if(res.data.error){
                    var a=0
                }else{
                    ...
                    this.setState({title:val["title"],
                        des:val["des"],route:val["route"],navigate:val["navigate"]
                    })
                }
    ...
    onsubmitForm=async (event)=>{
        ...
        await Axios.post(AppUrl.baseurl+"update-info",formData)
            .then(res=> {
                if(res.data.success){
